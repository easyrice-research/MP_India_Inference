...
class Graph(ModelDesc, Config):
    ...
    def _get_inputs(self):
        return [
            ...
            InputDesc(
                tf.float32, [None] + self.train_mask_shape + [2], "truemap-coded"
            ),
        ]
    ...
    def _build_graph(self, inputs):
        ...
        with argscope(
            ...
            predmap_coded = tf.concat(
                [obj_prob, cnt_prob], axis=-1, name="predmap-coded"
            )
        ...
        if is_training:
            ...
            def compute_scale_loss_bce(soft_list, one, overall_loss, type_name):
                ...
                for idx in range(len(soft_list)):
                    ...
                    if idx != len(soft_list) - 1:
                        loss_bce = tf.reduce_mean(
                            loss_bce, name="loss-bce-%s-us%d" % (type_name, idx)
                        )
                        overall_loss += aux_loss_dw * loss_bce
                        add_moving_summary(loss_bce)
                    else:
                        loss_bce = tf.reduce_mean(
                            loss_bce, name="loss-bce-%s-fused" % type_name
                        )
            ...
            overall_loss = compute_scale_loss_bce(
                soft_cnt_list, cnt_one, overall_loss, "cnt"
            )
            ...
            viz = tf.concat([orig_imgs, obj_pred, obj_true, cnt_pred, cnt_true], 2)
