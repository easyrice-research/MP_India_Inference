...
def decoder(name, i):
    ...
    with tf.variable_scope(name):
        ...
        with tf.variable_scope("u2"):
            ...
            u2x = Conv2D("conva", u2_sum, 128, 5, strides=1, padding="SAME")
    ...
    def _build_graph(self, inputs):
        ...
        with argscope(
            ...
            if self.type_classification:
                ...
                logi_class = Conv2D(
                    "conv_out_tp", tp, 5, 1, use_bias=True, activation=tf.identity
                )
        ...
        def get_gradient_hv(l, h_ch, v_ch):
            ...
            mh, mv = get_sobel_kernel(21)
            mh = tf.constant(mh, dtype=tf.float32)
            mv = tf.constant(mv, dtype=tf.float32)

            mh = tf.reshape(mh, [21, 21, 1, 1])
            mv = tf.reshape(mv, [21, 21, 1, 1])
        ...
        if get_current_tower_context().is_training:
            ...
            if self.type_classification:
                ...
                term_loss = (
                    dice_loss(soft_class[..., 0], one_type[..., 0])
