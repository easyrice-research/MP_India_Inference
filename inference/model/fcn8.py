...
class Graph(ModelDesc, Config):
    ...
    def _build_graph(self, inputs):
        ...
        with argscope(
            ...
            upsample1 = resize_op(
                d6, 2, 2, interp="bilinear", data_format="channels_first"
            )
            fuse1 = tf.add_n([upsample1, score_pool4])

            score_pool3 = Conv2D("score_pool3", p3, self.nr_classes, 1, use_bias=True)
            upsample2 = resize_op(
                fuse1, 2, 2, interp="bilinear", data_format="channels_first"
            )
            fuse2 = tf.add_n([upsample2, score_pool3])
            fuse2 = resize_op(
                fuse2, 8, 8, interp="bilinear", data_format="channels_first"
            )
