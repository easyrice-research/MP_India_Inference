...
class Graph(ModelDesc, Config):
    ...
    def _get_inputs(self):
        return [
            ...
            InputDesc(
                tf.float32, [None] + self.train_mask_shape + [1], "truemap-coded"
            ),
        ]
    ...
    def _build_graph(self, inputs):
        ...
        def unpool_with_argmax(pool, ind, name=None, ksize=[1, 2, 2, 1]):
            ...
            with tf.variable_scope(name):
                ...
                batch_range = tf.reshape(
                    tf.range(output_shape[0], dtype=ind.dtype), shape=[-1, 1, 1, 1]
                )
        ...
        def maxpool_with_argmax(x, kernel_size, stride, name=None, padding="SAME"):
            ...
            pool, argmax = tf.nn.max_pool_with_argmax(
                x, ksize, strides, padding=padding, name=name
            )
        ...
        with argscope(
            ...
            [Conv2D, MaxPooling, AvgPooling, BatchNorm], data_format=self.data_format
        ):
            ...
            d1 = Conv2D(
                "conv1_1", i, 64, 3, padding="same", strides=1, activation=BNReLU
            )
            d1 = Conv2D(
                "conv1_2", d1, 64, 3, padding="same", strides=1, activation=BNReLU
            )
            p1, p1_idx = maxpool_with_argmax(d1, 2, 2, name="pool1", padding="SAME")

            d2 = Conv2D(
                "conv2_1", p1, 128, 3, padding="same", strides=1, activation=BNReLU
            )
            d2 = Conv2D(
                "conv2_2", d2, 128, 3, padding="same", strides=1, activation=BNReLU
            )
            p2, p2_idx = maxpool_with_argmax(d2, 2, 2, name="pool2", padding="SAME")

            d3 = Conv2D(
                "conv3_1", p2, 256, 3, padding="same", strides=1, activation=BNReLU
            )
            d3 = Conv2D(
                "conv3_2", d3, 256, 3, padding="same", strides=1, activation=BNReLU
            )
            d3 = Conv2D(
                "conv3_3", d3, 256, 3, padding="same", strides=1, activation=BNReLU
            )
            p3, p3_idx = maxpool_with_argmax(d3, 2, 2, name="pool3", padding="SAME")

            d4 = Conv2D(
                "conv4_1", p3, 512, 3, padding="same", strides=1, activation=BNReLU
            )
            d4 = Conv2D(
                "conv4_2", d4, 512, 3, padding="same", strides=1, activation=BNReLU
            )
            d4 = Conv2D(
                "conv4_3", d4, 512, 3, padding="same", strides=1, activation=BNReLU
            )
            p4, p4_idx = maxpool_with_argmax(d4, 2, 2, name="pool4", padding="SAME")

            d5 = Conv2D(
                "conv5_1", p4, 512, 3, padding="same", strides=1, activation=BNReLU
            )
            d5 = Conv2D(
                "conv5_2", d5, 512, 3, padding="same", strides=1, activation=BNReLU
            )
            d5 = Conv2D(
                "conv5_3", d5, 512, 3, padding="same", strides=1, activation=BNReLU
            )
            ...
            us1 = Conv2D(
                "conv_us1_1", us1, 512, 3, padding="same", strides=1, activation=BNReLU
            )
            us1 = Conv2D(
                "conv_us1_2", us1, 512, 3, padding="same", strides=1, activation=BNReLU
            )
            us1 = Conv2D(
                "conv_us1_3", us1, 512, 3, padding="same", strides=1, activation=BNReLU
            )

            us2 = unpool_with_argmax(us1, ind=p4_idx, name="unpool_2")
            us2 = Conv2D(
                "conv_us2_1", us2, 512, 3, padding="same", strides=1, activation=BNReLU
            )
            us2 = Conv2D(
                "conv_us2_2", us2, 512, 3, padding="same", strides=1, activation=BNReLU
            )
            us2 = Conv2D(
                "conv_us2_3", us2, 256, 3, padding="same", strides=1, activation=BNReLU
            )

            us3 = unpool_with_argmax(us2, ind=p3_idx, name="unpool_3")
            us3 = Conv2D(
                "conv_us3_1", us3, 256, 3, padding="same", strides=1, activation=BNReLU
            )
            us3 = Conv2D(
                "conv_us3_2", us3, 256, 3, padding="same", strides=1, activation=BNReLU
            )
            us3 = Conv2D(
                "conv_us3_3", us3, 128, 3, padding="same", strides=1, activation=BNReLU
            )

            us4 = unpool_with_argmax(us3, ind=p2_idx, name="unpool_4")
            us4 = Conv2D(
                "conv_us4_1", us4, 128, 3, padding="same", strides=1, activation=BNReLU
            )
            us4 = Conv2D(
                "conv_us4_2", us4, 64, 3, padding="same", strides=1, activation=BNReLU
            )

            us5 = unpool_with_argmax(us4, ind=p1_idx, name="unpool_5")
            us5 = Conv2D(
                "conv_us5_1", us5, 64, 3, padding="same", strides=1, activation=BNReLU
            )
            us5 = Conv2D(
                "conv_us5_2",
                us5,
