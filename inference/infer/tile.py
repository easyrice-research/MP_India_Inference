...
import tqdm
from inference.dataloader.infer_loader import SerializeArray, SerializeFileList
from inference.misc.utils import (
    color_deconvolution,
    cropping_center,
    get_bounding_box,
    log_debug,
...
from inference.misc.viz_utils import colorize, visualize_instances_dict
...
class InferManager(base.InferManager):
    ...
    def process_file_list(self, run_args):
        ...
        assert os.path.isfile(self.input_dir), "Not Detected Any Files From Path"
        ...
        while len(file_path_list) > 0:
            ...
            while len(file_path_list) > 0:
                ...
                img = img[:7015, :4960]
                img = cv2.resize(img, (2480, 3508), interpolation=cv2.INTER_AREA)
            ...
            if proc_pool is not None:
                ...
                for future in as_completed(future_list):
                    ...
                    if future.exception() is not None:
                        ...
                        np.save(
                            "temp/pred_centroid_" + img_name + ".npy",
                            pred_inst_centroid,
                        )
