...
from inference.misc.utils import get_bounding_box, remove_small_objects
...
def __proc_np_hv(pred):
    ...
    h_dir_raw[h_dir_raw <-1] = -1
    v_dir_raw[v_dir_raw > 1] =  1

    # processing
    blb = np.array(blb_raw >= 0.5, dtype=np.int32)

    blb = measurements.label(blb)[0]
    blb = remove_small_objects(blb, min_size=150)
    ...
    ov_pix = overall[np.where(blb == 1)]
    cv_thes = ov_pix.mean()
    # print((ov_pix.mean(),ov_pix.max()))
    overall = np.array(overall >= cv_thes +0.015, dtype=np.int32)
    ...
    proced_pred = remove_small_objects(proced_pred, min_size=1500)
...
def process(pred_map, nr_types=None, return_centroids=False):
    ...
    if return_centroids or nr_types is not None:
        ...
        for inst_id in inst_id_list:
            ...
            pred_inst_centroid.append(inst_centroid)            
    ...
    return pred_inst, pred_type, pred_inst_centroid 
