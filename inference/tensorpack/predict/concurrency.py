            ...
            while not self.stopped():
                ...
                try:
                    ...
                    for f in futures:
                        f.cancel()
                    logger.warn(
                        "In PredictorWorkerThread id={}, call was cancelled.".format(
                            self.id
                        )
                    )
                    return
...
class MultiThreadAsyncPredictor(AsyncPredictorBase):
    ...
    def __init__(self, predictors, batch_size=5):
        ...
        self.threads = [
            PredictorWorkerThread(self.input_queue, f, id, batch_size=batch_size)
...
try:
    ...
    MultiThreadAsyncPredictor = create_dummy_class(
        "MultiThreadAsyncPredictor", "tornado.concurrent"
    )  # noqa
