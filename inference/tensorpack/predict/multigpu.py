        ...
        with self.graph.as_default():
            ...
            for idx, t in enumerate(towers):
                ...
                with tf.variable_scope(
                    tf.get_variable_scope(), reuse=idx > 0
                ), tf.device(device), PredictTowerContext(tower_name):
                    logger.info(
    ...
    def __init__(self, config, towers):
        ...
        with self.graph.as_default():
            ...
            for idx, t in enumerate(towers):
                ...
                inputs_desc = [
                    InputDesc(desc.type, desc.shape, tower_name + "_" + desc.name)
                ...
                with tf.variable_scope(
                    tf.get_variable_scope(), reuse=idx > 0
                ), tf.device("/gpu:{}".format(t)), PredictTowerContext(tower_name):
                    config.tower_func(*input.get_input_tensors())
