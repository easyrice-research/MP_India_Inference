...
class MultiProcessDatasetPredictor(DatasetPredictorBase):
    ...
    def __init__(self, config, dataset, nr_proc, use_gpu=True, ordered=True):
        ...
        if use_gpu:
            ...
            assert (
                len(gpus) >= self.nr_proc
            ), "nr_proc={} while only {} gpus available".format(self.nr_proc, len(gpus))
        else:
            gpus = ["-1"] * self.nr_proc
        # worker produces (idx, result) to outqueue
        self.outqueue = multiprocessing.Queue()
        self.workers = [
            MultiProcessQueuePredictWorker(i, self.inqueue, self.outqueue, self.config)
