...
def apply_default_prefetch(input_source_or_dataflow, trainer):
    ...
    if hasattr(trainer, "devices"):
        ...
        if len(towers) > 1:
            ...
            if isinstance(input, FeedfreeInput) and not isinstance(
                input, (StagingInput, DummyConstantInput)
            ):
...
def launch_train_with_config(config, trainer):
    ...
    trainer.setup_graph(
        model.get_inputs_desc(), input, model._build_graph_get_cost, model.get_optimizer
    )
...
def _check_unused_regularization():
    ...
    if unconsumed_reg:
        ...
        logger.warn(
            "Unconsumed regularization: {}".format(
