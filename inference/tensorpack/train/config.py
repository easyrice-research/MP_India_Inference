...
def DEFAULT_CALLBACKS():
    ...
    4. RunUpdateOps()
    """
    return [MovingAverageSummary(), ProgressBar(), MergeAllSummaries(), RunUpdateOps()]
...
class TrainConfig(object):
    ...
    def __init__(
        ...
        **kwargs
        ...
        # process data & model
        assert (
            data is None or dataflow is None
        ...
        if steps_per_epoch is None:
            try:
                ...
                logger.error(
                    "You must set `TrainConfig(steps_per_epoch)` if the size of your input is not available."
                )
                raise
        else:
...
class AutoResumeTrainConfig(TrainConfig):
    ...
    def __init__(self, always_resume=True, **kwargs):
        ...
        assert (
            found_sessinit == found_last_epoch
