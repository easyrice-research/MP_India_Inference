    ...
    def get_predictor(self, input_names, output_names, device=0):
        ...
        if tower is None:
            ...
            with tf.variable_scope(tf.get_variable_scope(), reuse=True), tf.device(
                device
            ), PredictTowerContext(tower_name, vs_name=vs_name):
                logger.info(
                    "Building graph for predict tower '{}' on device {} {}...".format(
                        tower_name,
                        device,
    ...
    def _make_get_grad_fn(self, input, get_cost_fn, get_opt_fn):
        ...
        def get_grad_fn():
            ...
            def compute_grad_from_inputs(*inputs):
                ...
                assert (
                    cost.shape.ndims == 0
                ), "Cost must be a scalar, but found {}!".format(cost)
