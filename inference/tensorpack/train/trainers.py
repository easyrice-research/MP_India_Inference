...
from ..graph_builder.distributed import (
...
class SyncMultiGPUTrainerReplicated(SingleCostTrainer):
    ...
    def __init__(self, gpus, average=True, mode=None, use_nccl=None):
        ...
        if use_nccl is not None:
            ...
            log_deprecated(
                "use_nccl option", "Use the `mode` option instead!", "2019-01-31"
            )
        if mode is None:
...
class DistributedTrainerBase(SingleCostTrainer):
    ...
    def initialize(self, session_creator, session_init):
        if (
            not isinstance(session_creator, NewSessionCreator)
            or session_creator.user_provided_config
        ):
            raise ValueError(
...
class DistributedTrainerReplicated(DistributedTrainerBase):
    ...
    def _setup_graph(self, input, get_cost_fn, get_opt_fn):
        ...
        cb = RunOp(
            lambda: initial_sync_op, run_before=True, run_as_trigger=False, verbose=True
        )
        cb.chief_only = False
        callbacks.append(cb)
...
class HorovodTrainer(SingleCostTrainer):
                ...
                if grad is not None:
                    if self._compression is not None and self._has_compression:
                        avg_grad = hvd.allreduce(
                            grad, average=self._average, compression=self._compression
                        )
                    else:
    ...
    def initialize(self, session_creator, session_init):
        ...
        if self.is_chief:
            ...
            logger.info(
                "Rank {} waiting for initialization broadcasting ...".format(self._rank)
            )
