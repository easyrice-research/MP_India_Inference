...
class QueueInput(FeedfreeInput):
    ...
    def _create_ema_callback(self):
        ...
        return RunOp(
            lambda: size_ema_op, run_before=False, run_as_trigger=False, run_step=True
        )
    ...
    def _setup(self, inputs):
        ...
        with self.cached_name_scope():
            if self.queue is None:
                self.queue = tf.FIFOQueue(
                    3000,
                    [x.dtype for x in self.input_placehdrs],
                    shapes=shapes,
                    name="input_queue",
                )
...
class TensorInput(FeedfreeInput):
    ...
    def __init__(self, get_tensor_fn, size=None):
        ...
        if not callable(get_tensor_fn):
            raise ValueError(
                "get_tensor_fn has to be a function! Got {}".format(get_tensor_fn)
            )
    ...
    def _get_input_tensors(self):
        ...
        assert len(ret) == len(
            self._desc
        ), "get_tensor_fn returns {} tensors but there are {} inputs".format(
...
class ZMQInput(TensorInput):
    ...
    def _setup(self, inputs_desc):
        assert len(inputs_desc) > 0, "ZMQInput has to be used with InputDesc!"
        self._desc = inputs_desc
...
class TFDatasetInput(FeedfreeInput):
    ...
    def __init__(self, dataset):
        ...
        if not isinstance(dataset, tf.data.Dataset):
            raise ValueError(
                "TFDatasetInput takes a tf.data.Dataset! Got {}".format(dataset)
            )
    ...
    def _get_input_tensors(self):
        ...
        assert len(ret) == len(
            desc_shapes
        ), "Dataset returns {} tensors but there are {} inputs!".format(
            len(ret), len(desc_shapes)
        )
...
class StagingInput(FeedfreeInput):
    ...
    class StagingCallback(Callback):
        ...
        def _setup_graph(self):
            ...
            self.fetches = tf.train.SessionRunArgs(fetches=[self.stage_op, unstage_op])
        ...
        def _prefill(self, sess):
            ...
            logger.info(
                "{} element{} put into StagingArea on each tower.".format(
    ...
    def _create_ema_callback(self):
        ...
        return RunOp(
            create_ema_op, run_before=False, run_as_trigger=False, run_step=True
        )
