...
def _check_name(tensor, name):
    ...
    assert (
        tensorname.split("/")[-1] == name
    ), "{} does not match {}, you may have name conflict somewhere!".format(
        tensor.name, name
    )
    ...
    def __call__(self, input_tensors):
        ...
        def post_process_model(model):
            ...
            for v in model.weights:
                ...
                if (
                    v.name not in old_trainable_names
                    and v.name in added_trainable_names
                ):
...
class KerasPhaseCallback(Callback):
    ...
    def _setup_graph(self):
        logger.info(
            "Using Keras learning phase {} in the graph!".format(
...
def setup_keras_trainer(
    trainer, get_model, inputs_desc, targets_desc, input, optimizer, loss, metrics
):
    """
    Args:
    ...
    def get_cost(*inputs):
        ...
        if isinstance(outputs, tf.Tensor):
            outputs = [outputs]
        assert len(outputs) == len(target_tensors), "len({}) != len({})".format(
            str(outputs), str(target_tensors)
        )
        assert len(outputs) == len(loss), "len({}) != len({})".format(
            str(outputs), str(loss)
        )
    ...
    trainer.setup_graph(inputs_desc + targets_desc, input, get_cost, lambda: optimizer)
...
class KerasModel(object):
    def __init__(self, get_model, inputs_desc, targets_desc, input, trainer=None):
        """
        Args:
    ...
    def fit(self, validation_data=None, **kwargs):
        ...
        if validation_data is not None:
            ...
            callbacks.insert(
                0,
                InferenceRunner(validation_data, ScalarStats(self._stats_to_inference)),
            )
