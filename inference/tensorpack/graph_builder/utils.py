...
class LeastLoadedDeviceSetter(object):
    ...
    def __call__(self, op):
        ...
        if var_size is None:
            logger.warn(
                "[LeastLoadedDeviceSetter] Shape of variable {} is not fully defined!".format(
                    op.name
                )
            )
            var_size = 0
...
class GradientPacker(object):
    ...
    def pack(self, grads):
        ...
        with cached_name_scope("GradientPacker", top_level=False):
            concat_grads = tf.concat(
                [tf.reshape(g, [-1]) for g in grads], 0, name="concatenated_grads"
            )
    ...
    def unpack(self, grad_packs):
        with cached_name_scope("GradientPacker", top_level=False):
            ...
            grads = [
                tf.reshape(g, shape) for g, shape in zip(flattened_grads, self._shapes)
            ]
            return grads
