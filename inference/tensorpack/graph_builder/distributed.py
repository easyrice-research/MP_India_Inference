    ...
    def _add_sync_queues_and_barrier(self, name, dependencies):
        ...
        with tf.device(
            self.sync_queue_devices[
                self._sync_queue_counter % len(self.sync_queue_devices)
            ]
        ):
            sync_queues = [
                tf.FIFOQueue(
...
class DistributedParameterServerBuilder(DataParallelBuilder, DistributedBuilderBase):
    ...
    def build(self, get_grad_fn, get_opt_fn):
        ...
        train_op = self._add_sync_queues_and_barrier(
            "all_workers_sync_barrier", [train_op]
        )
        return train_op
...
class DistributedReplicatedBuilder(DataParallelBuilder, DistributedBuilderBase):
    ...
    def _shadow_model_variables(shadow_vars):
        ...
        for v in model_vars:
            ...
            try:
                ...
                logger.warn(
                    "Model Variable {} also appears in other collections.".format(
                        stripped_var_name
                    )
                )
                continue
    ...
    def build(self, get_grad_fn, get_opt_fn):
        ...
        with tf.device(self.param_server_device):
            ...
            self._shadow_model_vars = (
                DistributedReplicatedBuilder._shadow_model_variables(self._shadow_vars)
            )

        # TODO add options to synchronize less
        main_fetch = tf.group(*var_update_ops, name="main_fetches")
        train_op = self._add_sync_queues_and_barrier("post_copy_barrier", [main_fetch])
    ...
    def _get_initial_sync_op(self):
        ...
        for v in self._shadow_vars:
            ...
            for i in range(self.nr_gpu):
                ...
                assert (
                    name in local_var_by_name
                ), "Shadow variable {} doesn't match a corresponding local variable!".format(
                    v.name
                )
