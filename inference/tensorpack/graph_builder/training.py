...
class DataParallelBuilder(GraphBuilder):
    def __init__(self, towers):
        ...
        if len(towers) > 1:
            logger.info(
                "[DataParallel] Training a model of {} towers.".format(len(towers))
            )
    ...
    def _check_grad_list(grad_list):
        ...
        if len(set(nvars)) != 1:
            names_per_gpu = [
                set([basename(k[1]) for k in grad_and_vars])
            ...
            for s in names_per_gpu:
                s -= inters
            logger.error(
                "Unique trainable variables on towers: " + pprint.pformat(names_per_gpu)
            )
            raise ValueError(
                "Number of gradients from each tower is different! " + str(nvars)
            )
        ...
        for idx, t in enumerate(towers):
            ...
            with tf.device(device), _maybe_reuse_vs(reuse), TrainTowerContext(
                ...
                if (
                    ...
                    logger.info(
                        "Building graph for training tower {} on device {} ...".format(
                            idx, device
                        )
                    )
                else:
        ...
        if self._mode in ["nccl", "hierarchical"]:
            ...
            if self._mode == "nccl":
                all_grads = allreduce_grads(
                    all_grads, average=self._average
                )  # #gpu x #param
            else:
            ...
            agg_grad_and_vars = aggregate_grads(
                grad_list, colocation=False, devices=["/cpu:0"], average=self._average
            )  # #param x 2
    ...
    def get_post_init_ops():
        ...
        for v in all_vars:
            ...
            if v.name.startswith("tower0"):
                ...
                log_failure(
                    v.name, "Name should not have prefix 'tower0' in this trainer!"
                )
            ...
            if prefix in realname:
                log_failure(
                    v.name,
                    "Prefix {} appears multiple times in its name!".format(prefix),
                )
                continue
