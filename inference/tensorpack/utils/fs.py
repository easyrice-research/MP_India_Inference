...
def download(url, dir, filename=None, expect_size=None):
    ...
    if os.path.isfile(fpath):
        if expect_size is not None and os.stat(fpath).st_size == expect_size:
            ...
            logger.warn(
                "File {} exists. Will overwrite with a new download!".format(filename)
            )
    ...
    if expect_size is not None and size != expect_size:
        logger.error(
            "File downloaded from {} does not match the expected size!".format(url)
        )
        logger.error(
...
def get_dataset_path(*args):
    ...
    if d is None:
        ...
        if execute_only_once():
            logger.warn(
                "Env var $TENSORPACK_DATASET not set, using {} for datasets.".format(d)
            )
