    ...
    def process(self):
        for idx, layer in enumerate(self.net.layers):
            ...
            if layer.type in self.processors:
                logger.info("Processing layer {} of type {}".format(name, layer.type))
        ...
        for i, layer in enumerate(self.net.layers):
            if layer.type == "BatchNorm":
                ...
                if bottom_name2 == bottom_name:
                    ...
                    logger.info(
                        "Merge {} and {} into one BatchNorm layer".format(name, name2)
                    )
                    return {
...
def load_caffe(model_desc, model_file):
    ...
    logger.info(
        "Model loaded from caffe. Params: " + ", ".join(sorted(param_dict.keys()))
    )
    return param_dict
