...
__all__ = ["total_timer", "timed_operation", "print_total_timer", "IterSpeedCounter"]
...
def timed_operation(msg, log_start=False):
    ...
    start = timer()
    yield
    logger.info("{} finished, time:{:.4f}sec.".format(msg, timer() - start))
...
class IterSpeedCounter(object):
    ...
    def __call__(self):
        ...
        t = timer() - self.start
        logger.info(
            "{}: {:.2f} sec, {} times, {:.3g} sec/time".format(
