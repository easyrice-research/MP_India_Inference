...
def MaxPooling(
    inputs, pool_size, strides=None, padding="valid", data_format="channels_last"
):
    """
...
def BilinearUpSample(x, shape):
    ...
    x = tf.pad(
        x,
        [[0, 0], [shape - 1, shape - 1], [shape - 1, shape - 1], [0, 0]],
        mode="SYMMETRIC",
    )
    out_shape = tf.shape(x) * tf.constant([1, shape, shape, 1], tf.int32)
    deconv = tf.nn.conv2d_transpose(
        x, weight_var, out_shape, [1, shape, shape, 1], "SAME"
    )
    edge = shape * (shape - 1)
