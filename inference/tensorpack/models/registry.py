...
def layer_register(log_shape=False, use_scope=True):
    ...
    def wrapper(func):
        ...
        def wrapped_func(*args, **kwargs):
            ...
            if use_scope:
                ...
                assert isinstance(name, six.string_types), (
                    'First argument for "{}" should be a string. '.format(func.__name__)
                ...
                if isinstance(args[0], six.string_types):
                    ...
                    name = None
            if not (
                isinstance(inputs, (tf.Tensor, tf.Variable))
                or (
            ...
            if name is not None:  # use scope
                with tf.variable_scope(name) as scope:
                    ...
                    if do_log_shape:
                        logger.info(
                            "{} input: {}".format(scope.name, get_shape_str(inputs))
                        )
                    ...
                    if do_log_shape:
                        ...
                        logger.info(
                            "{} output: {}".format(scope.name, get_shape_str(outputs))
                        )
        ...
        wrapped_func.symbolic_function = (
            func  # attribute to access the underlying function object
        )
