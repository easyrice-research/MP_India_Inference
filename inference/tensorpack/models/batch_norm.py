...
def get_bn_variables(n_out, use_scale, use_bias, beta_init, gamma_init):
    ...
    moving_mean = tf.get_variable(
        "mean/EMA", [n_out], initializer=tf.constant_initializer(), trainable=False
    )
...
def update_bn_ema(
    xn, batch_mean, batch_var, moving_mean, moving_var, decay, internal_update
):
    ...
    * ``variance/EMA``: the moving average of variance.
    ...
    if sync_statistics is None or not (training and ctx.is_training):
        ...
        with rename_get_variable(
            {"moving_mean": "mean/EMA", "moving_variance": "variance/EMA"}
        ):
            tf_args = dict(
                axis=axis,
                momentum=momentum,
        ...
        if new_shape is not None:
            ...
            xn = tf.nn.batch_normalization(
                inputs, batch_mean, batch_var, beta, gamma, epsilon
            )
...
def BatchRenorm(
    ...
    * ``moving_variance, renorm_stddev, renorm_stddev_weight``: See TF documentation.
    ...
    layer = tf.layers.BatchNormalization(
        ...
        scale=scale,
        renorm=True,
        renorm_clipping={"rmin": 1.0 / rmax, "rmax": rmax, "dmax": dmax},
