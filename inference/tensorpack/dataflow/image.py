...
def check_dtype(img):
    assert isinstance(
        img, np.ndarray
    ), "[Augmentor] Needs an numpy array, but got a {}!".format(type(img))
    assert not isinstance(img.dtype, np.integer) or (
        img.dtype == np.uint8
    ), "[Augmentor] Got image of type {}, use uint8 or floating points instead!".format(
        img.dtype
    )
        ...
        try:
            ...
            if not self.catch_exceptions:
                ...
                if self._nr_error % 100 == 0 or self._nr_error < 10:
                    logger.exception(
                        "Got {} augmentation errors.".format(self._nr_error)
                    )
...
class ImageFromFile(RNGDataFlow):
    ...
    def __init__(self, files, channel=3, resize=None, shuffle=False):
        ...
        self.imread_mode = (
            cv2.IMREAD_GRAYSCALE if self.channel == 1 else cv2.IMREAD_COLOR
        )
...
try:
    ...
    AugmentImageCoordinates = create_dummy_class(
        "AugmentImageCoordinates", "cv2"
    )  # noqa
