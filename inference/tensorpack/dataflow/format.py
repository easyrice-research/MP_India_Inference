...
class LMDBData(RNGDataFlow):
    ...
    def _set_keys(self, keys=None):
        def find_keys(txn, size):
            ...
            keys = []
            with timed_operation("Loading LMDB keys ...", log_start=True), get_tqdm(
                total=size
            ) as pbar:
        ...
        if self._shuffle:  # keys are necessary when shuffle is True
            if keys is None:
                ...
                if isinstance(keys, six.string_types):
                    self.keys = map(
                        lambda x: keys.format(x), list(np.arange(self._size))
                    )
                else:
                    self.keys = keys
...
class LMDBDataPoint(MapData):
    def __init__(self, *args, **kwargs):
        log_deprecated(
            "LMDBDataPoint", "Use LMDBSerializer.load() instead!", "2019-01-31"
        )
...
class TFRecordData(DataFlow):
    def __init__(self, path, size=None):
        log_deprecated(
            "TFRecordData", "Use TFRecordSerializer.load instead!", "2019-01-31"
        )
        self._path = path
        self._size = int(size)
