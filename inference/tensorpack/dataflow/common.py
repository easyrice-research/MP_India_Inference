...
class TestDataSpeed(ProxyDataFlow):
    ...
    def start_test(self):
        log_deprecated(
            "TestDataSpeed.start_test() was renamed to start()", "2019-03-30"
        )
        self.start()
...
class RandomMixData(RNGDataFlow):
    ...
    def __iter__(self):
        ...
        idxs = np.array(list(map(lambda x: np.searchsorted(sums, x, "right"), idxs)))
    ...
    def __init__(self, df_lists):
        ...
        try:
            ...
            for d in self.df_lists:
                assert (
                    len(d) == self._size
                ), "All DataFlow must have the same size! {} != {}".format(
                    len(d), self._size
                )
        except Exception:
            logger.info(
...
class PrintData(ProxyDataFlow):
    ...
    def _get_msg(self, dp):
        msg = [
            u"datapoint %i<%i with %i components consists of"
            % (self.cnt, self.num, len(dp))
        ]
