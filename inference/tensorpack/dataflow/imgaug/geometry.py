...
class Shift(TransformAugmentorBase):
    ...
    def __init__(
        self, horiz_frac=0, vert_frac=0, border=cv2.BORDER_REPLICATE, border_value=0
    ):
        """
        Args:
    ...
    def _get_augment_params(self, img):
        ...
        return WarpAffineTransform(
            mat, img.shape[1::-1], borderMode=self.border, borderValue=self.border_value
        )
...
class Affine(TransformAugmentorBase):
    ...
    def __init__(
        ...
        if scale is not None:
            assert (
                isinstance(scale, tuple) and len(scale) == 2
            ), "Argument scale should be a tuple of two floats, e.g (a, b)"

        if translate_frac is not None:
            assert (
                isinstance(translate_frac, tuple) and len(translate_frac) == 2
