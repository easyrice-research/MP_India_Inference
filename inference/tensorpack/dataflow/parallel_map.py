...
class _ParallelMapData(ProxyDataFlow):
    def __init__(self, ds, buffer_size, strict=False):
        ...
        self._buffer_occupancy = (
            0  # actual #elements in buffer, only useful in strict mode
        )
        self._strict = strict
    ...
    def _recv_filter_none(self):
        ...
        assert (
            ret is not None
        ), "[{}] Map function cannot return None when strict mode is used.".format(
            type(self).__name__
        )
        return ret
...
class MultiProcessMapDataZMQ(_ParallelMapData, _MultiProcessZMQDataFlow):
    ...
    def reset_state(self):
        ...
        self._procs = [
            MultiProcessMapDataZMQ._Worker(
