...
def _get_pipe_name(name):
    if sys.platform.startswith("linux"):
        ...
        filename = "{}/{}-pipe-{}".format(
            pipedir.rstrip("/"), name, str(uuid.uuid1())[:6]
        )
        assert not os.path.exists(
            filename
        ), "Pipe {} exists! You may be unlucky.".format(filename)
...
lead of failure on some of the code."
        ...
        self.procs = [
            MultiProcessPrefetchData._Worker(self.ds, self.queue, idx)
...
class PrefetchDataZMQ(_MultiProcessZMQDataFlow):
    ...
    def reset_state(self):
        ...
        self._procs = [
            PrefetchDataZMQ._Worker(self.ds, pipename, self._hwm, idx)
            for idx in range(self.nr_proc)
        ]
