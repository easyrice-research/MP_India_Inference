...
class CheckpointReaderAdapter(object):
    ...
    def __init__(self, reader):
        ...
        self._map = {
            k if k.endswith(":0") else k + ":0": v for k, v in six.iteritems(m)
        }
...
class MismatchLogger(object):
    ...
    def log(self):
        if len(self._names):
            logger.warn(
                "The following variables are in the {}, but not found in the {}: {}".format(
...
class SaverRestore(SessionInit):
    ...
    def __init__(self, model_path, prefix=None, ignore=[]):
        ...
        if model_path.endswith(".npy") or model_path.endswith(".npz"):
            logger.warn(
                "SaverRestore expect a TF checkpoint, but got a model path '{}'.".format(
                    model_path
                )
    ...
    def _match_vars(self, func):
        ...
        for v in graph_vars:
            ...
            if name in self.ignore and reader.has_tensor(name):
                logger.info(
                    "Variable {} in the graph will not be loaded from the checkpoint!".format(
                        name
                    )
                )
            else:
...
class DictRestore(SessionInit):
    ...
    def __init__(self, variable_dict):
        ...
        self._prms = {
            get_op_tensor_name(n)[1]: v for n, v in six.iteritems(variable_dict)
        }
    ...
    def _run_init(self, sess):
        ...
        logger.info(
            "Variables to restore from dict: {}".format(", ".join(map(str, intersect)))
        )
