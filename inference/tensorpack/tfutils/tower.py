...
class BaseTowerContext(object):
    ...
    def __init__(self, ns_name, vs_name=""):
        ...
        if len(vs_name):
            assert len(
                ns_name
            ), "TowerContext(vs_name) cannot be used with an empty name!"
    ...
    def __enter__(self):
        ...
        assert ns == self._name, (
            "Name conflict: name_scope inside tower '{}' becomes '{}'!".format(
                self._name, ns
            )
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if not self.has_own_variables:
            ...
            assert (
                len(diff_trainable_vars) == 0
            ), "New TRAINABLE_VARIABLES shouldn't be created in {}: ".format(
                self._name
            ) + ", ".join(
                [k.name for k in diff_trainable_vars]
            )
...
class TrainTowerContext(BaseTowerContext):
    ...
    def __init__(self, ns_name, vs_name="", index=0, total=1):
        ...
        if self.index > 0:
            assert self.total > self.index, "(index, total) = ({}, {})".format(
        ...
        if self.has_own_variables:
            assert not vs.reuse, "Cannot create tower {} under reuse=True!".format(
                ns_name
            )
...
class TowerFuncWrapper(object):
    ...
    def __init__(self, tower_fn, inputs_desc):
        ...
        assert len(set(self._inputs_desc_names)) == len(
            self._inputs_desc_names
