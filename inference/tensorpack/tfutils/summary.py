...
def add_tensor_summary(x, types, name=None, collections=None, main_tower_only=True):
    ...
    SUMMARY_TYPES_DIC = {
        "scalar": lambda: tf.summary.scalar(
            name + "-summary", x, collections=collections
        ),
        "histogram": lambda: tf.summary.histogram(
            name + "-histogram", x, collections=collections
        ),
        "sparsity": lambda: tf.summary.scalar(
            name + "-sparsity", tf.nn.zero_fraction(x), collections=collections
        ),
        "mean": lambda: tf.summary.scalar(
            name + "-mean", tf.reduce_mean(x), collections=collections
        ),
...
def add_moving_summary(*args, **kwargs):
    ...
    for x in args:
        ...
        assert (
            x.get_shape().ndims == 0
        ), "add_moving_summary() only accepts scalar tensor! Got one with {}".format(
            x.get_shape()
        )
        ...
        with tf.name_scope(None):
            ...
            with tf.name_scope(ns):  # reuse VS&NS so that EMA_1 won't appear
                ema_op = moving_averages.assign_moving_average(
                    ema_var, c, decay, zero_debias=True, name=name + "_EMA_apply"
                )
            ema_ops.append(ema_op)
        with tf.name_scope(None):
            tf.summary.scalar(
                name + "-summary", ema_op, collections=summ_coll
