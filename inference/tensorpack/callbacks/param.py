        ...
        if ret is not None and ret != self._last_value:
            if self.epoch_num != self._last_epoch_set:
                ...
                if self._last_value is None:
                    logger.info(
                        "[HyperParamSetter] At global_step={}, {} is set to {:.6f}".format(
                            self.global_step, self.param.readable_name, ret
                        )
                    )
                else:
                    logger.info(
                        "[HyperParamSetter] At global_step={}, {} changes from {:.6f} to {:.6f}".format(
                            self.global_step,
                            self.param.readable_name,
                            self._last_value,
                            ret,
                        )
                    )
...
class HumanHyperParamSetter(HyperParamSetter):
    ...
    def _get_value_to_set(self):
        ...
        try:
            ...
            logger.warn(
                "Cannot find {} in {}".format(self.param.readable_name, self.file_name)
            )
            return None
...
class ScheduledHyperParamSetter(HyperParamSetter):
    ...
    def _get_value_to_set(self):
        ...
        if self.interp is None:
            ...
            return None
        v = (refnum - laste) * 1.0 / (e - laste) * (v - lastv) + lastv
        return v
...
class StatMonitorParamSetter(HyperParamSetter):
    ...
    def _get_value_to_set(self):
        try:
            ...
            logger.warn(
                "[StatMonitorParamSetter] No history data available for key '{}'.".format(
                    self.stat_name
                )
            )
            return None
        ...
        self.history.clear()
        logger.info(
            "[StatMonitorParamSetter] Triggered, history of {}: ".format(self.stat_name)
            + ",".join([str(round(x, 3)) for x in values])
        )
