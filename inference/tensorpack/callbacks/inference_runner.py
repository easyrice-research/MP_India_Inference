...
__all__ = ["InferenceRunnerBase", "InferenceRunner", "DataParallelInferenceRunner"]
...
class InferenceRunnerBase(Callback):
    ...
    def _before_train(self):
        ...
        if self._size > 0:
            ...
            logger.warn(
                "[InferenceRunner] Got an InputSource with unknown size! Will iterate until OutOfRangeError!"
            )
...
class InferenceRunner(InferenceRunnerBase):
    ...
    def _setup_graph(self):
        if self._tower_func is None:
            assert (
                self.trainer.tower_func is not None
        ...
        logger.info(
            "[InferenceRunner] Building tower '{}' on device {} {}...".format(
                self._tower_name,
                self._device,
                "with variable scope '{}'".format(vs_name) if vs_name else "",
            )
        )
        with tf.variable_scope(tf.get_variable_scope(), reuse=True), tf.device(
            self._device
    ...
    def _trigger(self):
        ...
        with _inference_context(), tqdm.tqdm(
            total=self._size, **get_tqdm_kwargs()
        ) as pbar:
...
class DataParallelInferenceRunner(InferenceRunnerBase):
    ...
    class InferencerToHookDataParallel(InferencerToHook):
        def __init__(self, inf, fetches, size):
            ...
            """
            super(
                DataParallelInferenceRunner.InferencerToHookDataParallel, self
            ).__init__(inf, fetches)
    ...
    def _before_train(self):
        ...
        self._parallel_hooked_sess = HookedSession(
            self.trainer.sess, self._hooks_parallel
        )
