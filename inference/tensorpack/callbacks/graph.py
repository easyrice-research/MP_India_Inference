...
class RunOp(Callback):
    ...
    def __init__(
        self, op, run_before=True, run_as_trigger=True, run_step=False, verbose=False
    ):
        """
        Args:
    ...
    def __init__(self, names):
        ...
        def fn(*args):
            ...
            for name, val in zip(self._names, args):
                dic[name] = val
            fname = os.path.join(dir, "DumpTensor-{}.npz".format(self.global_step))
            np.savez(fname, **dic)
...
class DumpTensorAsImage(Callback):
    ...
    def _dump_image(self, im, idx=None):
        ...
        fname = os.path.join(
            self.log_dir,
            self.prefix
            + "-ep{:03d}{}.png".format(self.epoch_num, "-" + str(idx) if idx else ""),
        )
        res = im * self.scale
...
class CheckNumerics(Callback):
    ...
    def _setup_graph(self):
        ...
        ops = [
            tf.check_numerics(v, "CheckNumerics['{}']".format(v.op.name)).op
            for v in vars
        ]
