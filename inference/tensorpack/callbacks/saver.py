...
class MinSaver(Callback):
    ...
    def _trigger(self):
        ...
        if self.best is None or (
            curr_val > self.best[1] if self.reverse else curr_val < self.best[1]
        ):
    ...
    def _save(self):
        ...
        if not path.endswith(str(self.best[0])):
            logger.warn(
                "[MinSaver] New {} '{}' found at global_step={}, but the latest checkpoint is {}.".format(
        ...
        newname = os.path.join(
            ...
            self.filename
            or (
                "max-" + self.monitor_stat
                if self.reverse
                else "min-" + self.monitor_stat
            ),
        )
        files_to_copy = tf.gfile.Glob(path + "*")
        for file_to_copy in files_to_copy:
            tf.gfile.Copy(
                file_to_copy, file_to_copy.replace(path, newname), overwrite=True
            )
        logger.info(
            "Model at global_step={} with {} {}={:.5g} saved.".format(
...
class MaxSaver(MinSaver):
    ...
    def __init__(self, monitor_stat, filename=None, checkpoint_dir=None):
        ...
        super(MaxSaver, self).__init__(
            monitor_stat, True, filename=filename, checkpoint_dir=checkpoint_dir
        )
