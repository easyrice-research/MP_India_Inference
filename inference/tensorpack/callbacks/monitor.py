    ...
    def _before_train(self):
        ...
        if stats is not None:
            ...
            if epoch is None or epoch == starting_epoch:
                ...
                backup_fname = (
                    JSONWriter.FILENAME + "." + datetime.now().strftime("%m%d-%H%M%S")
                )
                backup_fname = os.path.join(logger.get_logger_dir(), backup_fname)

                logger.warn(
                    "Now, we will train with starting_epoch={} and backup old json to {}".format(
...
class ScalarPrinter(MonitorBase):
    ...
    def __init__(
        self, enable_step=False, enable_epoch=True, whitelist=None, blacklist=None
    ):
        """
        Args:
