...
class PeriodicTrigger(ProxyCallback):
    ...
    def __init__(
        self, triggerable, every_k_steps=None, every_k_epochs=None, before_train=False
    ):
        """
        Args:
        ...
        if before_train is False:
            assert (every_k_epochs is not None) or (
                every_k_steps is not None
...
class PeriodicRunHooks(ProxyCallback):
    ...
    def __init__(self, callback, every_k_steps):
        ...
        log_deprecated(
            "PeriodicRunHooks", "Use PeriodicCallback instead!", "2019-02-28"
        )
...
class PeriodicCallback(EnableCallbackIf):
    ...
    def predicate(self):
        ...
        if self._epoch_k is not None:
            if (
                self.local_step == self.trainer.steps_per_epoch - 1
                and self.epoch_num == self.trainer.max_epoch
            ):
                return True
        return False
