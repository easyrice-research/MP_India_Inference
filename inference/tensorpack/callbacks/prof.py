...
class GraphProfiler(Callback):
    ...
    def _write_meta(self, metadata):
        fname = os.path.join(self._dir, "runmetadata-{}.pb".format(self.global_step))
        with open(fname, "wb") as f:
            f.write(metadata.SerializeToString())

    def _write_tracing(self, metadata):
        tl = timeline.Timeline(step_stats=metadata.step_stats)
        fname = os.path.join(self._dir, "chrome-trace-{}.json".format(self.global_step))
        with open(fname, "w") as f:
            f.write(
                tl.generate_chrome_trace_format(show_dataflow=True, show_memory=True)
            )
